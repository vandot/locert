...
proc genCA*() =
  var genCAcmd = "$# req -x509 -sha256 -nodes -days 3650 -newkey rsa:2048 -keyout $# -out $# -subj \"/C=LO/ST=Dev/L=Local/O=Dev/OU=Local/CN=locert\"" %
      [opensslBin, joinPath("locert-tmp", "locertCA.key"), joinPath(
      "locert-tmp", "locertCA.pem")]
...
proc genCSR*(domain: string) =
  var genCSRcmd = "$# req -sha256 -nodes -newkey rsa:2048 -keyout locert.key -out $# -subj \"/C=LO/ST=Dev/L=Local/O=Dev/OU=Local/CN=$#\"" %
      [opensslBin, joinPath("locert-tmp", "locert.csr"), domain]
...
proc genCRT*(domain: string) =
  genExt(domain)
  var genCRTcmd = "$# x509 -req -in $# -CA $# -CAkey $# -CAcreateserial -out locert.crt -days 825 -sha256 -extfile $#" %
      [opensslBin, joinPath("locert-tmp", "locert.csr"), joinPath("locert-tmp",
      "locertCA.pem"), joinPath("locert-tmp", "locertCA.key"), joinPath(
      "locert-tmp", "locert.ext")]
